...
class SiameseNetwork(nn.Module):
    def __init__(self):
        ...
        self.conv5 = nn.Conv2d(64, 32, kernel_size=3, padding=0)
        self.conv6 = nn.Conv2d(32, 16, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm2d(16)
        # Fully connected layers
        self.fc1 = nn.Linear(16 * 12 * 12, 56)  
        self.fc2 = nn.Linear(56, 36)
    ...
    def forward_one(self, x):
        ...
        #x = F.max_pool2d(x, 2)  
        x = F.relu(self.conv2(x)) # 16 * 112 * 112
        x = F.max_pool2d(x, 2) # 16 * 56 * 56 
        
        x = F.relu(self.conv3(x)) # 32 * 54 * 54
        x = F.relu(self.conv4(x)) # 64 * 52 * 52
        x = F.max_pool2d(x, 2) # 64 * 26 * 26
        
        x = F.relu(self.conv5(x)) # 32 * 24 * 24
        x = F.relu(self.conv6(x)) # 16 * 24 * 24
...
if __name__ == '__main__':
    ...
    val_loader_aug = DataLoader(val_dataset_aug, batch_size=batch_size, num_workers=cores, shuffle=False, pin_memory=True)
    test_loader_aug = DataLoader(test_dataset_aug, batch_size=batch_size, num_workers=cores, shuffle=False, pin_memory=True)
